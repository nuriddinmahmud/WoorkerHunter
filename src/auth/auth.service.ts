...
export class AuthService {
  private ACCESS_SECRET = process.env.JWT_SECRET;
  private REFRESH_SECRET = process.env.JWT_SECRET;
  private OTP_SECRET = process.env.JWT_SECRET;
...
async register(RegisterUser: RegisterDTO) {
  ...
  try {
    ...
    if (!['USER_FIZ', 'USER_YUR'].includes(role)) {
    ...
    if (role === 'USER_YUR' && !company) {
      throw new BadRequestException('Company details are required for USER_YUR role.');
    }
    ...
    try {
      ...
      if (!user) {
        throw new NotFoundException(
          'User not found with the provided phone number.Please register first!',
        );
      }
  ...
  async me(req: Request) {
    ...
    try {
      ...
      if (userRole === 'USER_FIZ' || userRole === 'COMPANY') {
